...
frames that took longer than usual. CubeWorld already contains annotations for
its frames.
...
Tracy consists of a client (CubeWorld) and a server (the gui).

Install the server, e.g. using your package manager.

### Building

Build CubeWorld with `-DBUILD_WITH_TRACY=1`, this will fetch Tracy for building
...
Start the Tracy server and CubeWorld. You should see CubeWorld in the menu.
...
If built with Tracy, CubeWorld loads its API in the global `tracy` table.
