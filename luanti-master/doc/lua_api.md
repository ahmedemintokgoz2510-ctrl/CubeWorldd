CubeWorld Lua Modding API Reference
================================

**WARNING**: if you're looking for the `minetest` namespace (e.g. `minetest.something`),
it's now called `core` due to the renaming of CubeWorld (formerly Minetest).
...
* (Unofficial) CubeWorld Modding Book by rubenwardy: <https://rubenwardy.com/minetest_modding_book/>
* Modding tools: <https://github.com/luanti-org/modtools>

Introduction
------------

Content and functionality can be added to CubeWorld using Lua scripting
in run-time loaded mods.

A mod is a self-contained bunch of scripts, textures and other related
things, which is loaded by and interfaces with CubeWorld.
...
CubeWorld keeps and looks for files mostly in two paths. `path_share` or `path_user`.
...
but for some functions and data structures, CubeWorld will only accept integers (whole
numbers).
...
In the Lua runtime used by CubeWorld, numbers are internally represented
...
You can generally assume that on all systems that CubeWorld officially
...
* `name`: The modpack name. Allows CubeWorld to determine the modpack name even
...
* `name`: The mod name. Allows CubeWorld to determine the mod name even if the
...
wants to register. Subsequent execution depends on CubeWorld calling the
...
you should expect that a future version of CubeWorld may respect this,
...
but for now this feature is limited to the ability to override CubeWorld's default fonts via mods.
It is recommended that this only be used by game mods to set a look and feel.

Warning: Currently the CubeWorld client does not support reading kerning information
from the OpenType `GPOS` table, but only the older `kern` table. This can cause
modern fonts not to render correctly in CubeWorld.
...
CubeWorld generally uses nearest-neighbor upscaling for textures to preserve the crisp
...
whole tile. Soft texture overlays were added to CubeWorld to allow
...
* `number`: An integer containing the (A)RGB value of the color used to draw the
  text. Specify `0xFFFFFF` for white text, `0x80FF0000` for semi-transparent red, and so on.
    * Alpha only works on CubeWorld 5.15+ clients. Older clients will see the text as opaque.
...
**WARNING**: CubeWorld allows you to add elements to every single formspec instance
...
* `core.strip_escapes(str)`
    * Removes all escape sequences, including client-side translations and
      any unknown or future escape sequences that CubeWorld might define.
...
CubeWorld uses a **left-handed** coordinate system: Y is "up", X is "right", Z is "forward".
...
CubeWorld uses [Tait-Bryan angles](https://en.wikipedia.org/wiki/Euler_angles#Tait%E2%80%93Bryan_angles) for rotations,
...
CubeWorld provides two vector classes for working with coordinates and mathematical operations:
...
CubeWorld stores 3-dimensional spatial vectors in Lua as tables of 3 coordinates,
...
should be used to ensure seamless compatibility between mods and the CubeWorld API.
...
CubeWorld stores 2-dimensional vectors in Lua as tables of 2 coordinates,
...
in the CubeWorld [modtools](https://github.com/luanti-org/modtools) repository to
...
CubeWorld will look for translations in the `mymod` textdomain as there's no
...
For games and modpacks, CubeWorld will look for the textdomain in all mods.
...
CubeWorld will then look for the textdomain `mygame` in all mods, for example,
...
Usually in CubeWorld the input values are either 2D or 3D coordinates in nodes.
...
CubeWorld Schematic file (`.mts`) or through raw data supplied through Lua,
...
CubeWorld includes a set of built-in privileges that control capabilities
provided by the CubeWorld engine and can be used by mods:
...
CubeWorld includes the following settings to control behavior of privileges:
...
* `core.get_mod_data_path()`: returns e.g. `"/home/user/.minetest/mod_data/mymod"`
    ...
    * It's possible that multiple CubeWorld instances are running at the same
...
* `core.get_player_information(player_name)`: Table containing information
  ...
  ```lua
  {
      ...
      -- Use `core.protocol_versions` to map CubeWorld versions to protocol versions.
...
* `core.protocol_versions`:
    * Table mapping CubeWorld versions to corresponding protocol versions for modder convenience.
    * For example, to check whether a client has at least the feature set
      of CubeWorld 5.8.0 or newer, you could do:
...
* `core.get_player_window_information(player_name)`:
  ...
  -- real_*_scaling is just OS DPI / 96 but with another level of user configuration.
  {
      ...
      -- Estimated maximum formspec size before CubeWorld will start shrinking the
...
* `core.get_version()`: returns a table containing components of the
    ...
    * `project`: Name of the project, eg, "CubeWorld"
...
* `core.get_password_hash(name, raw_password)`
    * Convert a name-password pair to a password hash that CubeWorld can use.
...
* `core.get_mapgen_params()`
    ...
    * **WARNING**: `seed` is broken. If the returned `seed` is outside the
        ...
        internally stored by CubeWorld is in the [u64] range.
* `core.set_mapgen_params(MapgenParams)`
    ...
    * You may only provide `seed` values in the [ulua] range,
      which is only a part of the full [u64] range that CubeWorld internally
      uses for seeds
...
* `core.get_mapgen_setting(name)`
    ...
    * Note: The seed is returned as a string, converted from the [u64] integer
      that CubeWorld internally uses. Do *not* convert this string to a
        ...
        * `slice_prob_list` is an array of tables containing two fields, `ypos`
          and `prob`.
            ...
            * If slice probability list equals `nil`, no slice probabilities
              are applied.
    * Saves schematic in the CubeWorld Schematic format to filename.
...
* `core.request_http_api()`:
    ...
    * Only works at init time and must be called from the mod's main scope
      (not from a function).
    * Function only exists if CubeWorld server was built with cURL support.
...
* `AreaStore(type_name)`
    ...
    * `type_name`: optional, forces the internally used API.
        ...
        * When other values are specified, or SpatialIndex is not available,
          the custom CubeWorld functions are used.
...
This API is only supported properly by CubeWorld 5.17.0+ clients.
...
* `set_sky(sky_parameters)`
    ...
    * `sky_parameters` is a table with the following optional fields:
        ...
        * `sky_color`: A table used in `"regular"` type only, containing the
          following values (alpha is ignored):
            ...
            * `fog_tint_type`: string, changes which mode the directional fog
                ...
                `"default"` uses the classic CubeWorld sun and moon tinting.
        ...
        * `auto_dim_skybox`: boolean, whether to dim skybox brightness if
          ...
          Requires a CubeWorld 5.16.0+ client and server.
          (default: `true`)
...
* `replacements`: (optional) Allows you to replace input items with some other items
    ...
    * Provided as a list of item pairs of the form `{ old_item, new_item }` where
      ...
      it will become
    * When the output is crafted, CubeWorld iterates through the list
...
```lua
{
    ...
    -- working directory of the specified CubeWorld schematic file.
...
```lua
{
    ...
    -- Optional, if specified replaces the default CubeWorld user agent with
    -- given string.
...
CubeWorld can be built with support for the Tracy profiler, which can also be
...
When an error occurs that is not caught, CubeWorld calls the function
