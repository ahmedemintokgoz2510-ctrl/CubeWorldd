...
World metadata. This file is created and updated by CubeWorld when creating a
...
Custom keys should be namespaced to avoid conflicts with CubeWorld and other
...
CubeWorld maps consist of `MapBlock`s, each holds 16x16x16 nodes.
...
Initially, CubeWorld stored maps in a format called the "sectors" format.
...
Eventually CubeWorld outgrew this directory structure, as filesystems were
...
* Indicates if the light is correct at the sides of a map block.
  ...
  If these flags are false, CubeWorld will automatically recompute light
...
* Example: if the block at `(0, 0, 0)` has `lighting_complete = 0b1111111111111110`,
  CubeWorld will correct lighting in the day light bank when the block at
